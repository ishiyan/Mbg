...
func main() {
	...
	printMicGo("mics.go", ams, ics, ecs, tzmics)
	printMicTestGo("mics_test.go", ams, ics, ecs)
}
...
func printMic(w io.Writer, m *market) {
	if m.mic == cxanMic ||
		strings.Contains(m.name, "INDUSTRIEL") ||
		strings.Contains(m.name, "PRODUCTOS") ||
		strings.Contains(m.name, "PORTUGUES") ||
		strings.Contains(m.name, "TRANSILVANIA") ||
		strings.Contains(m.name, "REGISTERD") ||
		strings.Contains(m.comments, "REGISTERD") {
		printf(w, "//nolint:misspell\n")
	}
	...
	if m.comments != "" &&
		...
		printf(w, "//\n")
		printf(w, "// Comments: %v", m.comments)
...
//nolint:funlen
func printMicGo(filename string, ms []*mic, ics []string, ecs []string, tzmics []*tzmic) {
	var b bytes.Buffer
	...
	printf(&b, "// MIC is a Market Identifier Code according to ISO 10383.\n")
	printf(&b, "// See https://www.iso20022.org/market-identifier-codes.\n")
	printf(&b, "type MIC string\n\n")
	...
	printf(&b, "//nolint:exhaustive,misspell,funlen\n")
	printf(&b, "// OperationalMIC returns an operational MIC related to this predefined segment MIC.\n")
	printf(&b, "//\n")
	printf(&b, "// Returns this MIC if it is not a predefined segment.\n")
	printf(&b, "func (m MIC) OperationalMIC() MIC {\n")
	printf(&b, "\tswitch m {\n")

	for _, m := range ms {
		if len(m.segs) > 0 {
			printf(&b, "\tcase %v:\n", concatenateMics(m.segs))
			printf(&b, "\t\treturn %v\n", m.oper.mic)
		}
	}
	...
	printf(&b, "//nolint:gomnd,exhaustive,misspell,funlen\n")
	printf(&b, "// TimeZoneSeconds returns a seconds east of UTC for this predefined MIC.\n")
	printf(&b, "//\n")
	printf(&b, "// Returns zero if this MIC is not predefined.\n")
	printf(&b, "func (m MIC) TimeZoneSeconds() int {\n")
	printf(&b, "\tswitch m {\n")

	for _, t := range tzmics {
		if t.tzsec != 0 {
			printf(&b, "\tcase %v:\n", concatenateMics(t.markets))
			printf(&b, "\t\treturn %v\n", t.tzsec)
		}
	}
	...
	printf(&b, "//nolint:misspell\n")
	printf(&b, "// IsPredefined indicates if this MIC is predefined.\n")
	printf(&b, "func (m MIC) IsPredefined() bool {\n")
	printf(&b, "\tswitch m {\n")
	printf(&b, "\tcase %v,\n", concatenateMics(tzmics[0].markets))

	l := len(tzmics) - 1
	for i := 1; i <= l; i++ {
		if i < l {
			printf(&b, "\t\t%v,\n", concatenateMics(tzmics[i].markets))
		} else {
			printf(&b, "\t\t%v:\n", concatenateMics(tzmics[i].markets))
		}
	}
...
//nolint:funlen
func printMicTestGo(filename string, ms []*mic, ics []string, ecs []string) {
	var b bytes.Buffer
	...
	printf(&b, "func TestOperationalMIC(t *testing.T) {\n")
	printf(&b, "\tt.Parallel()\n\n")
	printf(&b, "\ttests := []struct {\n")
	printf(&b, "\t\tmic         MIC\n")
	printf(&b, "\t\toperational MIC\n")
	printf(&b, "\t}{\n")
	printf(&b, "\t\t{MIC(\"FOO\"), MIC(\"FOO\")}, // unknown\n")

	for _, m := range ms {
		printf(&b, "\t\t{%v, %v},\n", safeMic(m.oper.mic), safeMic(m.oper.micOp))
		...
		for _, s := range m.segs {
			...
			printf(&b, "\t\t{%v, %v},\n", safeMic(s.mic), safeMic(s.micOp))
		}
	}

	printf(&b, "\t}\n\n")
	printf(&b, "\tfor _, tt := range tests {\n")
	printf(&b, "\t\texp := tt.operational\n")
	printf(&b, "\t\tact := tt.mic.OperationalMIC()\n\n")
	printf(&b, "\t\tif exp != act {\n")
	printf(&b, "\t\t\tt.Errorf(\"%%v.OperationalMIC(): expected '%%v', actual '%%v'\", tt.mic, exp, act)\n")
	printf(&b, "\t\t}\n")
	printf(&b, "\t}\n")
	printf(&b, "}\n\n")

	printf(&b, "//nolint:funlen\n")
	printf(&b, "func TestTimeZoneSeconds(t *testing.T) {\n")
	printf(&b, "\tt.Parallel()\n\n")
	printf(&b, "\ttests := []struct {\n")
	printf(&b, "\t\tmic    MIC\n")
	printf(&b, "\t\tseconds int\n")
	printf(&b, "\t}{\n")
	printf(&b, "\t\t{MIC(\"FOO\"), 0}, // unknown\n")

	for _, m := range ms {
		printf(&b, "\t\t{%v, %v},\n", safeMic(m.oper.mic), m.oper.tzsec)
		...
		for _, s := range m.segs {
			...
			printf(&b, "\t\t{%v, %v},\n", safeMic(s.mic), s.tzsec)
		}
	}

	printf(&b, "\t}\n\n")
	printf(&b, "\tfor _, tt := range tests {\n")
	printf(&b, "\t\texp := tt.seconds\n")
	printf(&b, "\t\tact := tt.mic.TimeZoneSeconds()\n\n")
	printf(&b, "\t\tif exp != act {\n")
	printf(&b, "\t\t\tt.Errorf(\"%%v.TimeZoneSeconds(): expected '%%v', actual '%%v'\", tt.mic, exp, act)\n")
	printf(&b, "\t\t}\n")
	printf(&b, "\t}\n")
	printf(&b, "}\n\n")

	printf(&b, "//nolint:funlen\n")
	printf(&b, "func TestIsPredefined(t *testing.T) {\n")
	printf(&b, "\tt.Parallel()\n\n")
	printf(&b, "\ttests := []struct {\n")
	printf(&b, "\t\tmic        MIC\n")
	printf(&b, "\t\tpredefined bool\n")
	printf(&b, "\t}{\n")
	printf(&b, "\t\t{MIC(\"FOO\"), false}, // unknown\n")
	...
	printf(&b, "\t}\n\n")
	printf(&b, "\tfor _, tt := range tests {\n")
	printf(&b, "\t\texp := tt.predefined\n")
	printf(&b, "\t\tact := tt.mic.IsPredefined()\n\n")
	printf(&b, "\t\tif exp != act {\n")
	printf(&b, "\t\t\tt.Errorf(\"%%v.IsPredefined(): expected '%%v', actual '%%v'\", tt.mic, exp, act)\n")
	printf(&b, "\t\t}\n")
	printf(&b, "\t}\n")
	printf(&b, "}\n\n")
