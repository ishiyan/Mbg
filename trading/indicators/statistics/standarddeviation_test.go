...
func testStDevTime() time.Time {
	return time.Date(2021, time.April, 1, 0, 0, 0, 0, &time.Location{})
}

// testStdevInput is the input data to test standard deviation calculations.
func testStdevInput() []float64 {
	return []float64{
...
// testStdevLength5PopulationExpected is the Excel (STDEV.P) output of standard deviation
// based on population variance of length 5.
//nolint:dupl
func testStdevLength5PopulationExpected() []float64 {
	return []float64{
...
// testStdevLength5SampleExpected is the Excel (STDEV.P) output of standard deviation
// based on the unbiased estimation of the sample variance of length 5.
//nolint:dupl
func testStdevLength5SampleExpected() []float64 {
	return []float64{
		math.NaN(), math.NaN(), math.NaN(), math.NaN(),
		1.4373960832004500, 0.4989188310737520, 0.9882775419890900, 1.1251411022622900, 1.6233953615801700,
		1.5286374651957200, 2.0861405753208500, 2.8426717186477900, 3.4243659997144000, 3.5750164335286600,
		3.8675331284941800, 3.8134508650302500, 3.7470678536690500, 1.5102714987710000, 1.5494499023847100,
		1.5920646971778500, 1.2157096692878600, 1.5309898758646300, 2.5625290242258700, 2.7889724810402800,
		2.4259003483243100, 2.3384808102697800, 1.3467386531914800, 2.9751840279216300, 2.9856586375538600,
		2.8949365278016000, 1.8215295770313500, 1.4984366853491000, 0.6950953172047700, 1.4508730130511100,
		1.6159184694779600, 1.2708530599561900, 1.2980052002977500, 1.6128600373250000, 1.7155429169799300,
		1.4331625867290800, 1.3391060450912800, 0.8365300353244950, 2.3512268074347900, 2.8846607252846900,
		3.1747972376200700, 2.2215478837963400, 1.0088830457491100, 1.0669582934679300, 0.9587596153363990,
		0.9272108713771640, 1.1265345090142600, 1.0058976091034300, 2.6544961857196200, 3.1249159988710100,
		3.0290997012313700, 2.3735511370096900, 1.1393967702253700, 1.4588077323622900, 2.2741591852814500,
		2.0299495067611900, 1.6840820941985000, 1.2214213032365200, 1.4336736030212700, 1.6421632074796900,
		2.1140730829372900, 1.9991073007720200, 0.8830770068346250, 0.9383229721156790, 1.4745168700289600,
		1.7499771427078700, 1.7253572093917300, 2.4166660919539500, 2.9690516162572900, 2.7335905874874500,
		2.0293206991503300, 5.6255459735033700, 7.6871992624102100, 8.8223022788839000, 8.0386149304466600,
		3.6023561733954100, 2.3994567093406800, 1.5805797986814800, 1.8121499662003700, 1.8121499662003700,
		1.6039544881323800, 0.7973205127174390, 1.4479623613892700, 1.9676966229579200, 2.4162636445553700,
		2.9816157532452100, 5.8928161773467900, 5.9794466717247300, 5.1535788535735000, 3.7097843603099100,
		1.9427673561185900, 1.2647944497031900, 1.6790860609271900, 2.8326145166612400, 3.0891192110373500,
		6.0992811051795300, 5.8893488604428900, 3.7231277173903100, 1.5500580634286000, 1.4911170309536400,
		1.0993270668913800, 0.7906200098656740, 0.9607653199403050, 3.6265231282869300, 5.7397935502943000,
...
func TestStandardDeviationMetadata(t *testing.T) {
	...
	t.Run("population standard deviation", func(t *testing.T) {
		...
		check("Outputs[0].Name", "stdev.p(7)", act.Outputs[0].Name)
		check("Outputs[0].Description",
			"Standard deviation based on estimation of the population variance stdev.p(7)", act.Outputs[0].Description)
	})
	...
	t.Run("sample standard deviation", func(t *testing.T) {
		...
		check("Outputs[0].Kind", int(StandardDeviationValue), act.Outputs[0].Kind)
		check("Outputs[0].Type", output.Scalar, act.Outputs[0].Type)
		check("Outputs[0].Name", "stdev.s(7)", act.Outputs[0].Name)
		check("Outputs[0].Description",
			"Standard deviation based on unbiased estimation of the sample variance stdev.s(7)", act.Outputs[0].Description)
	})
...
func TestNewStandardDeviation(t *testing.T) { //nolint: funlen
	...
	t.Run("length > 1, unbiased", func(t *testing.T) {
		...
		checkSuccessful(&params, "stdev.s(5)",
			"Standard deviation based on unbiased estimation of the sample variance stdev.s(5)")
	})
	...
	t.Run("length > 1, biased", func(t *testing.T) {
		...
		checkSuccessful(&params, "stdev.p(5)",
			"Standard deviation based on estimation of the population variance stdev.p(5)")
	})
	...
	t.Run("length = 1", func(t *testing.T) {
		...
		checkErroneous(&params, []StandardDeviationOutput{StandardDeviationValue}, errlen)
	})
	...
	t.Run("invalid bar component", func(t *testing.T) {
		...
		params := VarianceParams{
			Length: length, IsUnbiased: true, BarComponent: data.BarComponent(9999), QuoteComponent: qc, TradeComponent: tc,
		}

		checkErroneous(&params, []StandardDeviationOutput{StandardDeviationValue}, errbc)
	})
	...
	t.Run("invalid quote component", func(t *testing.T) {
		...
		params := VarianceParams{
			Length: length, IsUnbiased: true, BarComponent: bc, QuoteComponent: data.QuoteComponent(9999), TradeComponent: tc,
		}

		checkErroneous(&params, []StandardDeviationOutput{StandardDeviationValue}, errqc)
	})
	...
	t.Run("invalid trade component", func(t *testing.T) {
		...
		params := VarianceParams{
			Length: length, IsUnbiased: true, BarComponent: bc, QuoteComponent: qc, TradeComponent: data.TradeComponent(9999),
		}

		checkErroneous(&params, []StandardDeviationOutput{StandardDeviationValue}, errtc)
	})
	...
	t.Run("invalid output", func(t *testing.T) {
		...
		checkErroneous(&params, []StandardDeviationOutput{StandardDeviationValue, StandardDeviationOutput(0)}, errout)
	})
}

func testStandardDeviationCreate(length int, unbiased bool, outputs []StandardDeviationOutput) *StandardDeviation {
	params := VarianceParams{
