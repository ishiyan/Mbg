...
// Input data taken from test_FRAMA.xsl reference implementation, Mid-Price, D5…D256, 252 entries.

func testFractalAdaptiveMovingAverageInputMid() []float64 { //nolint:dupl
	return []float64{
...
// Input data taken from test_FRAMA.xsl reference implementation, High, B5…B256, 252 entries.

func testFractalAdaptiveMovingAverageInputHigh() []float64 { //nolint:dupl
	return []float64{
...
// Input data taken from test_FRAMA.xsl reference implementation, Low, C5…C256, 252 entries.

func testFractalAdaptiveMovingAverageInputLow() []float64 { //nolint:dupl
	return []float64{
...
// Expected data taken from test_FRAMA.xsl reference implementation, FRAMA, R5…R256, 252 entries.
...
// Expected data taken from test_FRAMA.xsl reference implementation, FDIM, O5…O256, 252 entries.
...
func TestFractalAdaptiveMovingAverageUpdateEntity(t *testing.T) { //nolint: funlen,cyclop
	...
	t.Run("update bar", func(t *testing.T) {
		...
		b := data.Bar{Time: time, Close: inp, High: inp, Low: inp}
		frama := testFractalAdaptiveMovingAverageCreate(l, a)
	...
	t.Run("update quote", func(t *testing.T) {
		...
		q := data.Quote{Time: time, Bid: inp, Ask: inp}
		frama := testFractalAdaptiveMovingAverageCreate(l, a)
...
func TestNewFractalAdaptiveMovingAverage(t *testing.T) { //nolint: funlen
	...
	t.Run("odd lengths", func(t *testing.T) {
		...
		frama, err := NewFractalAdaptiveMovingAverage(&params)
		check("err == nil", true, err == nil)
		checkInstance(frama, "frama(18, 0.010)", l+1, a)
	})
