...
namespace Tests.Indicators
{
    ...
    public class TriangularMovingAverageTest
    {
        ...
        /// <summary>
        /// Taken from TA-Lib (http://ta-lib.org/) tests, test_ma.c, TA_SREF_close_daily_ref_0_PRIV[252].
        /// /***************/
        /// /*  TRIMA TEST */
        /// /***************/
        ...
        {
            // Values from index=0 to index=8 are double.NaN.
            93.6043,  // Index=9 value.
            93.4252,  // Index=10 value.
            109.1850, // Index=250 value.
            109.1407  // Index=251 (last) value.
        };
        private readonly List<double> expected9 = new List<double>
        {
            // Values from index=0 to index=7 are double.NaN.
            93.8176, // Index=8 value.
            109.1312 // Index=251 (last) value.
        };
        private readonly List<double> expected12 = new List<double>
        {
            // Values from index=0 to index=10 are double.NaN.
            93.5329, // Index=11 value.
            109.1157 // Index=251 (last) value.
        };

        /// <summary>
        /// Taken from TA-Lib (http://ta-lib.org/) tests, test_TRIMA.xsl, TRIMA(12), Q4…Q255, 252 entries.
        /// </summary>
        private readonly List<double> expectedXls = new List<double>
        {
                  ...
                  double.NaN, 93.5329761904762, 93.6096428571428, 93.5933333333333, 93.6425000000000, 93.7965476190476, 93.8471428571429, 93.6536904761905, 93.2340476190476, 92.6722619047619,
        ...
        {
            var target = new TriangularMovingAverage(5);
            Assert.AreEqual("TRIMA", target.Name);
        }
        #endregion
        ...
        {
            var target = new TriangularMovingAverage(4);
            Assert.AreEqual("TRIMA4", target.Moniker);
        }
        #endregion
        ...
        {
            var target = new TriangularMovingAverage(3);
            Assert.AreEqual("Triangular Moving Average", target.Description);
        }
        #endregion
        ...
        #region TaLibTest9
        /// <summary>
        /// A TA-Lib data test 9.
        /// </summary>
        [TestMethod]
        public void TaLibTest9()
        {
            int count = input.Count;
            const int dec = 4;
            double d;
            var target = new TriangularMovingAverage(9);
            for (int i = 0; i < 8; i++)
            {
            ...
            d = target.Update(input[8]);
            Assert.IsFalse(double.IsNaN(d));
            Assert.IsTrue(target.IsPrimed);
            Assert.AreEqual(expected9[0], Math.Round(d, dec));
            for (int i = 9; i < count; i++)
                d = target.Update(input[i]);
            Assert.IsFalse(double.IsNaN(d));
            Assert.IsTrue(target.IsPrimed);
            Assert.AreEqual(expected9[1], Math.Round(d, dec));
        }
        #endregion

        #region TaLibTest10
        /// <summary>
        /// A TA-Lib data test 10.
        /// </summary>
        [TestMethod]
        public void TaLibTest10()
        {
            int count = input.Count;
            const int dec = 4;
            double d;
            var target = new TriangularMovingAverage(10);
            for (int i = 0; i < 9; i++)
            {
                d = target.Update(input[i]);
                Assert.IsTrue(double.IsNaN(d));
            ...
            Assert.IsFalse(double.IsNaN(d));
            Assert.IsTrue(target.IsPrimed);
            Assert.AreEqual(expected10[0], Math.Round(d, dec));
            d = target.Update(input[10]);
            Assert.IsFalse(double.IsNaN(d));
            Assert.IsTrue(target.IsPrimed);
            Assert.AreEqual(expected10[1], Math.Round(d, dec));
            --count;
            for (int i = 11; i < count; i++)
                d = target.Update(input[i]);
            Assert.IsFalse(double.IsNaN(d));
            Assert.IsTrue(target.IsPrimed);
            Assert.AreEqual(expected10[2], Math.Round(d, dec));
            d = target.Update(input[count]);
            Assert.IsFalse(double.IsNaN(d));
            Assert.IsTrue(target.IsPrimed);
            Assert.AreEqual(expected10[3], Math.Round(d, dec));
        }
        #endregion

        #region TaLibTest12
        /// <summary>
        /// A TA-Lib data test 12.
        /// </summary>
        [TestMethod]
        public void TaLibTest12()
        {
            int count = input.Count;
            const double epsilon = 0.001;
            double d;
            var target = new TriangularMovingAverage(12);
            for (int i = 0; i < 11; i++)
            {
                d = target.Update(input[i]);
                Assert.IsTrue(double.IsNaN(d));
            ...
            Assert.IsFalse(double.IsNaN(d));
            Assert.IsTrue(target.IsPrimed);
            Assert.IsTrue(Math.Abs(expected12[0] - d) < epsilon);
            for (int i = 12; i < count; i++)
                d = target.Update(input[i]);
            Assert.IsFalse(double.IsNaN(d));
            Assert.IsTrue(target.IsPrimed);
            Assert.IsTrue(Math.Abs(expected12[1] - d) < epsilon);
        }
        #endregion
        ...
        {
            var target = new TriangularMovingAverage(11);
            Assert.AreEqual(11, target.Length);
            target = new TriangularMovingAverage(22);
        ...
        #region TriangularMovingAverageConstructorTest
        /// <summary>
        /// A test for Constructor.
        /// </summary>
        [TestMethod]
        public void TriangularMovingAverageConstructorTest()
        {
            var target = new TriangularMovingAverage(5);
            ...
            target = new TriangularMovingAverage(7, OhlcvComponent.MedianPrice);
        ...
        public void TriangularMovingAverageConstructorTest2()
        {
            var target = new TriangularMovingAverage(1);
            Assert.IsFalse(target.IsPrimed);
        }
        ...
        [TestMethod]
        [ExpectedException(typeof(ArgumentOutOfRangeException))]
        public void TriangularMovingAverageConstructorTest3()
        {
            var target = new TriangularMovingAverage(0);
            Assert.IsFalse(target.IsPrimed);
        }
        ...
        [TestMethod]
        [ExpectedException(typeof(ArgumentOutOfRangeException))]
        public void TriangularMovingAverageConstructorTest4()
        {
            var target = new TriangularMovingAverage(-7);
            Assert.IsFalse(target.IsPrimed);
        }
        #endregion

        #region SerializationTest
        private static void SerializeTo(TriangularMovingAverage instance, string fileName)
        {
            var dcs = new DataContractSerializer(typeof(TriangularMovingAverage), null, 65536, false, true, null);
        ...
        private static TriangularMovingAverage DeserializeFrom(string fileName)
        {
            var fs = new FileStream(fileName, FileMode.Open);
            XmlDictionaryReader reader = XmlDictionaryReader.CreateTextReader(fs, new XmlDictionaryReaderQuotas());
            var ser = new DataContractSerializer(typeof(TriangularMovingAverage), null, 65536, false, true, null);
            var instance = (TriangularMovingAverage)ser.ReadObject(reader, true);
            reader.Close();
            fs.Close();
            return instance;
        }
        ...
        public void SerializationTest()
        {
            ...
            const int dec = 4;
            double d;
            var source = new TriangularMovingAverage(10);
            for (int i = 0; i < 9; i++)
            {
            ...
            d = source.Update(input[9]);
            Assert.IsFalse(double.IsNaN(d));
            Assert.IsTrue(source.IsPrimed);
            Assert.AreEqual(expected10[0], Math.Round(d, dec));
            d = source.Update(input[10]);
            Assert.IsFalse(double.IsNaN(d));
            Assert.IsTrue(source.IsPrimed);
            Assert.AreEqual(expected10[1], Math.Round(d, dec));
            const string fileName = "TriangularMovingAverageTest_1.xml";
            SerializeTo(source, fileName);
            TriangularMovingAverage target = DeserializeFrom(fileName);
            Assert.AreEqual(10, target.Length);
            Assert.IsTrue(target.IsPrimed);
            Assert.AreEqual(expected10[1], Math.Round(target.Value, dec));
            Assert.AreEqual("TRIMA", target.Name);
            Assert.AreEqual("Triangular Moving Average", target.Description);
            --count;
            for (int i = 11; i < count; i++)
