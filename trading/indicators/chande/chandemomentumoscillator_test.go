...
func TestChandeMomentumOscillatorMetadata(t *testing.T) {
	...
	check("Type", indicator.ChandeMomentumOscillator, act.Type)
	check("len(Outputs)", 1, len(act.Outputs))
	check("Outputs[0].Kind", int(ChandeMomentumOscillatorValue), act.Outputs[0].Kind)
	check("Outputs[0].Type", output.Scalar, act.Outputs[0].Type)
	check("Outputs[0].Name", "cmo(5)", act.Outputs[0].Name)
...
func TestNewChandeMomentumOscillator(t *testing.T) { //nolint: funlen
	...
	t.Run("length > 0", func(t *testing.T) {
		t.Parallel()
		params := ChandeMomentumOscillatorParams{
			Length: length, BarComponent: bc, QuoteComponent: qc, TradeComponent: tc,
		}

		cmo, err := NewChandeMomentumOscillator(&params)
		...
		check("tradeFunc == nil", false, cmo.tradeFunc == nil)
	})
...
func testChandeMomentumOscillatorCreate(length int) *ChandeMomentumOscillator {
	...
	cmo, _ := NewChandeMomentumOscillator(&params)
