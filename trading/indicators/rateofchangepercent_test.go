...
func TestRateOfChangePercentMetadata(t *testing.T) {
	...
	check("Type", indicator.RateOfChangePercent, act.Type)
	check("len(Outputs)", 1, len(act.Outputs))
	check("Outputs[0].Kind", int(RateOfChangePercentValue), act.Outputs[0].Kind)
	check("Outputs[0].Type", output.Scalar, act.Outputs[0].Type)
	check("Outputs[0].Name", "rocp(5)", act.Outputs[0].Name)
...
func TestNewRateOfChangePercent(t *testing.T) { //nolint: funlen
	...
	t.Run("length > 0", func(t *testing.T) {
		t.Parallel()
		params := RateOfChangePercentParams{
			Length: length, BarComponent: bc, QuoteComponent: qc, TradeComponent: tc,
		}

		rocp, err := NewRateOfChangePercent(&params)
		check("err == nil", true, err == nil)
		check("name", "rocp(5)", rocp.name)
		check("description", "Rate of Change percent rocp(5)", rocp.description)
		...
		check("barFunc == nil", false, rocp.barFunc == nil)
		check("quoteFunc == nil", false, rocp.quoteFunc == nil)
		check("tradeFunc == nil", false, rocp.tradeFunc == nil)
	})
	...
	t.Run("length = 1", func(t *testing.T) {
		...
		rocp, err := NewRateOfChangePercent(&params)
		check("err == nil", true, err == nil)
		check("name", "rocp(1)", rocp.name)
		check("description", "Rate of Change percent rocp(1)", rocp.description)
		check("primed", false, rocp.primed)
...
func testRateOfChangePercentCreate(length int) *RateOfChangePercent {
	...
	rocp, _ := NewRateOfChangePercent(&params)
