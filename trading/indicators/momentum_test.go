...
func TestMomentumMetadata(t *testing.T) {
	...
	check("Type", indicator.Momentum, act.Type)
	check("len(Outputs)", 1, len(act.Outputs))
	check("Outputs[0].Kind", int(MomentumValue), act.Outputs[0].Kind)
	check("Outputs[0].Type", output.Scalar, act.Outputs[0].Type)
	check("Outputs[0].Name", "mom(5)", act.Outputs[0].Name)
...
func TestNewMomentum(t *testing.T) { //nolint: funlen
	...
	t.Run("length > 0", func(t *testing.T) {
		...
		mom, err := NewMomentum(&params)
		check("err == nil", true, err == nil)
		check("name", "mom(5)", mom.name)
		check("description", "Momentum mom(5)", mom.description)
		...
		check("barFunc == nil", false, mom.barFunc == nil)
		check("quoteFunc == nil", false, mom.quoteFunc == nil)
		check("tradeFunc == nil", false, mom.tradeFunc == nil)
	})
	...
	t.Run("length = 1", func(t *testing.T) {
		...
		mom, err := NewMomentum(&params)
		check("err == nil", true, err == nil)
		check("name", "mom(1)", mom.name)
		check("description", "Momentum mom(1)", mom.description)
		check("primed", false, mom.primed)
