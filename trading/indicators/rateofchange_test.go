...
func TestRateOfChangeMetadata(t *testing.T) {
	...
	check("Type", indicator.RateOfChange, act.Type)
	check("len(Outputs)", 1, len(act.Outputs))
	check("Outputs[0].Kind", int(RateOfChangeValue), act.Outputs[0].Kind)
	check("Outputs[0].Type", output.Scalar, act.Outputs[0].Type)
	check("Outputs[0].Name", "roc(5)", act.Outputs[0].Name)
...
func TestNewRateOfChange(t *testing.T) { //nolint: funlen
	...
	t.Run("length > 0", func(t *testing.T) {
		t.Parallel()
		params := RateOfChangeParams{
			Length: length, BarComponent: bc, QuoteComponent: qc, TradeComponent: tc,
		}

		roc, err := NewRateOfChange(&params)
		check("err == nil", true, err == nil)
		check("name", "roc(5)", roc.name)
		check("description", "Rate of Change roc(5)", roc.description)
		...
		check("barFunc == nil", false, roc.barFunc == nil)
		check("quoteFunc == nil", false, roc.quoteFunc == nil)
		check("tradeFunc == nil", false, roc.tradeFunc == nil)
	})
	...
	t.Run("length = 1", func(t *testing.T) {
		...
		roc, err := NewRateOfChange(&params)
		check("err == nil", true, err == nil)
		check("name", "roc(1)", roc.name)
		check("description", "Rate of Change roc(1)", roc.description)
		check("primed", false, roc.primed)
