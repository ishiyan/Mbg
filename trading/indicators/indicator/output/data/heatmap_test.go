...
var testHeatmapTime = time.Date(2021, time.April, 1, 0, 0, 0, 0, &time.Location{})
...
func TestNewHeatmap(t *testing.T) {
	...
	h := NewHeatmap(testHeatmapTime, p1, p2, p3, p4, p5, []float64{p6, p7})

	check("Time", testHeatmapTime, h.Time)
...
func TestNewEmptyHeatmap(t *testing.T) {
	t.Parallel()
	...
	h := NewEmptyHeatmap(testHeatmapTime, p1, p2, p3)
...
func TestHeatmapIsEmpty(t *testing.T) {
	...
	h := createTestHeatmap()
	if h.IsEmpty() {
		t.Errorf("expected not empty, actual is empty")
	}

	h.Values = nil
	if !h.IsEmpty() {
		t.Errorf("expected empty (Values is nil), actual not empty")
	}

	h.Values = []float64{}
	if !h.IsEmpty() {
		t.Errorf("expected empty (Values length is 0), actual not empty")
	}
}
...
func createTestHeatmap() Heatmap {
	return Heatmap{
