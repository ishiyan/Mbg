...
func TestAdaptiveMovingAverageUpdate(t *testing.T) {
	t.Parallel()
...
func TestAdaptiveMovingAverageMetadata(t *testing.T) {
	...
	checkInstance := func(act indicator.Metadata, name string) {
		...
		check("Outputs[0].Description", "Kaufman adaptive moving average "+name, act.Outputs[0].Description)
	}
	...
	t.Run("length = 10, fastest len = 2, slowest len = 30", func(t *testing.T) {
		...
		kama := testAdaptiveMovingAverageCreateLength(l, f, s)
		act := kama.Metadata()
		checkInstance(act, "kama(10, 2, 30)")
	})
	...
	t.Run("length = 10, fastest α = 0.666666666, slowest α = 0.064516129", func(t *testing.T) {
		...
		act := kama.Metadata()
		checkInstance(act, "kama(10, 0.6667, 0.0645)")
	})
}
