...
func testAdaptiveMovingAverageExpected() []float64 { //nolint:dupl
	return []float64{
...
func TestAdaptiveMovingAverageUpdate(t *testing.T) { //nolint: funlen
	t.Parallel()
	...
	input := testAdaptiveMovingAverageInput()
	...
	t.Run("value, length = 10, fastest = 2, slowest = 30 (kama.xls)", func(t *testing.T) {
		...
		kama := testAdaptiveMovingAverageCreateLength(l, f, s)

		exp := testAdaptiveMovingAverageExpected()
	...
	t.Run("efficiency ratio, length = 10 (kama.xls)", func(t *testing.T) {
		...
		exp := testAdaptiveMovingAverageExpectedEr()
...
func TestAdaptiveMovingAverageUpdateEntity(t *testing.T) { //nolint: funlen
	t.Parallel()
	...
	time := testAdaptiveMovingAverageTime()
	...
	t.Run("update scalar", func(t *testing.T) {
		...
		kama := testAdaptiveMovingAverageCreateLength(l, fastest, slowest)
...
func TestAdaptiveMovingAverageIsPrimed(t *testing.T) {
	t.Parallel()
...
func TestAdaptiveMovingAverageMetadata(t *testing.T) {
	t.Parallel()
	...
	checkInstance := func(act indicator.Metadata, name, nameEr string) {
		const (
			two  = 2
			desc = "Kaufman adaptive moving average "
		)

		check("Type", indicator.KaufmanAdaptiveMovingAverage, act.Type)
		check("len(Outputs)", two, len(act.Outputs))
		check("Outputs[0].Kind", int(AdaptiveMovingAverageValue), act.Outputs[0].Kind)
		check("Outputs[0].Type", output.Scalar, act.Outputs[0].Type)
		check("Outputs[0].Name", name, act.Outputs[0].Name)
		check("Outputs[0].Description", desc+name, act.Outputs[0].Description)
		check("Outputs[1].Kind", int(AdaptiveMovingAverageValueEr), act.Outputs[1].Kind)
		check("Outputs[1].Type", output.Scalar, act.Outputs[1].Type)
		check("Outputs[1].Name", nameEr, act.Outputs[1].Name)
		check("Outputs[1].Description", desc+nameEr, act.Outputs[1].Description)
	}
	...
	t.Run("length = 10, fastest len = 2, slowest len = 30", func(t *testing.T) {
		...
		kama := testAdaptiveMovingAverageCreateLength(l, f, s)
		act := kama.Metadata()
		checkInstance(act, "kama(10, 2, 30)", "er(10)")
	})
	...
	t.Run("length = 10, fastest α = 0.666666666, slowest α = 0.064516129", func(t *testing.T) {
		...
		kama := testAdaptiveMovingAverageCreateAlpha(l, f, s)
		act := kama.Metadata()
		checkInstance(act, "kama(10, 0.6667, 0.0645)", "er(10)")
	})
}

func TestNewAdaptiveMovingAverage(t *testing.T) { //nolint: funlen, maintidx
	t.Parallel()
	...
	const (
		...
		errvol  = "invalid Kaufman adaptive moving average parameters: volume factor should be in range [0, 1]"
	...
	checkInstance := func(kama *AdaptiveMovingAverage,
		name string, length int, lenFastest int, lenSlowest int, af float64, as float64,
	) {
		if math.IsNaN(af) {
			af = two / float64(lenFastest+1)
		}

		if math.IsNaN(as) {
			as = two / float64(lenSlowest+1)
		}

		check("name", name, kama.name)
		check("description", "Kaufman adaptive moving average "+name, kama.description)
		...
		check("barFunc == nil", false, kama.barFunc == nil)
		check("quoteFunc == nil", false, kama.quoteFunc == nil)
		check("tradeFunc == nil", false, kama.tradeFunc == nil)
	}
	...
	t.Run("efficiency ratio length > 1, (fast,slow) len = (2,30)", func(t *testing.T) {
		...
		params := AdaptiveMovingAverageLengthParams{
			EfficiencyRatioLength: l, FastestLength: f, SlowestLength: s,
			BarComponent: bc, QuoteComponent: qc, TradeComponent: tc,
		}

		kama, err := NewAdaptiveMovingAverageLength(&params)
		check("err == nil", true, err == nil)
		checkInstance(kama, "kama(10, 2, 30)", l, f, s, math.NaN(), math.NaN())
	})
	...
	t.Run("efficiency ratio length = 0, error", func(t *testing.T) {
		...
		params := AdaptiveMovingAverageLengthParams{
			EfficiencyRatioLength: 0, FastestLength: f, SlowestLength: s,
			BarComponent: bc, QuoteComponent: qc, TradeComponent: tc,
		}
	...
	t.Run("0 ≤ α ≤ 1, both smoothing factors", func(t *testing.T) {
		...
		params := AdaptiveMovingAverageSmoothingFactorParams{
			EfficiencyRatioLength: l, FastestSmoothingFactor: f, SlowestSmoothingFactor: s,
			BarComponent: bc, QuoteComponent: qc, TradeComponent: tc,
		}

		kama, err := NewAdaptiveMovingAverageSmoothingFactor(&params)
		check("err == nil", true, err == nil)
		checkInstance(kama, "kama(10, 0.6667, 0.3333)", l, 0, 0, f, s)
	})
...
func testAdaptiveMovingAverageCreateLength(
	...
	kama, _ := NewAdaptiveMovingAverageLength(&params)

	return kama
}

func testAdaptiveMovingAverageCreateAlpha(
	...
	kama, _ := NewAdaptiveMovingAverageSmoothingFactor(&params)
