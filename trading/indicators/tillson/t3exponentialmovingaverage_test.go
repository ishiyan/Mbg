...
func TestT3ExponentialMovingAverageUpdate(t *testing.T) { //nolint: funlen, cyclop
	...
	t.Run("length = 5, firstIsAverage = true", func(t *testing.T) {
		...
		const (
			i250value = 109.032 // Index=2500 value.
...
func TestT3ExponentialMovingAverageUpdateEntity(t *testing.T) { //nolint: funlen, dupl
	t.Parallel()
...
func TestT3ExponentialMovingAverageMetadata(t *testing.T) {
	...
	checkInstance := func(act indicator.Metadata, name string) {
		check("Type", indicator.T3ExponentialMovingAverage, act.Type)
		check("len(Outputs)", 1, len(act.Outputs))
		check("Outputs[0].Kind", int(T3ExponentialMovingAverageValue), act.Outputs[0].Kind)
		check("Outputs[0].Type", output.Scalar, act.Outputs[0].Type)
		check("Outputs[0].Name", name, act.Outputs[0].Name)
		check("Outputs[0].Description", "T3 exponential moving average "+name, act.Outputs[0].Description)
	}
	...
	t.Run("alpha = 2/11 = 0.18181818..., v=0.3333333, firstIsAverage = false", func(t *testing.T) {
		...
		act := t3.Metadata()
		checkInstance(act, "t3(0.1818 (10), 0.33)")
	})
}
...
func TestNewT3ExponentialMovingAverage(t *testing.T) { //nolint: funlen, maintidx
	...
	checkInstance := func(
		t3 *T3ExponentialMovingAverage, name string, length int, alpha float64, firstIsAverage bool,
	) {
		check("name", name, t3.name)
		check("description", "T3 exponential moving average "+name, t3.description)
		...
		check("barFunc == nil", false, t3.barFunc == nil)
		check("quoteFunc == nil", false, t3.quoteFunc == nil)
		check("tradeFunc == nil", false, t3.tradeFunc == nil)
	}
	...
	t.Run("length > 1, firstIsAverage = false", func(t *testing.T) {
		...
		checkInstance(t3, "t3(10, 0.70)", length, alpha, false)
	})
	...
	t.Run("epsilon < α ≤ 1", func(t *testing.T) {
		...
		check("err == nil", true, err == nil)
		checkInstance(t3, "t3(0.1818 (10), 0.70)", length, alpha, true)
	})
	...
	t.Run("0 < α < epsilon", func(t *testing.T) {
		...
		t3, err := NewT3ExponentialMovingAverageSmoothingFactor(&params)
		check("err == nil", true, err == nil)
		checkInstance(t3, "t3(0.0000 (199999999), 0.70)", length, alpha, false)
	})
	...
	t.Run("α = 0", func(t *testing.T) {
		...
		t3, err := NewT3ExponentialMovingAverageSmoothingFactor(&params)
		check("err == nil", true, err == nil)
		checkInstance(t3, "t3(0.0000 (199999999), 0.70)", length, alpha, true)
	})
	...
	t.Run("α = 1", func(t *testing.T) {
		...
		t3, err := NewT3ExponentialMovingAverageSmoothingFactor(&params)
		check("err == nil", true, err == nil)
		checkInstance(t3, "t3(1.0000 (1), 0.70)", length, alpha, true)
	})
	...
	t.Run("volume factor = 0.5", func(t *testing.T) {
		...
		t3, err := NewT3ExponentialMovingAverageLength(&params)
		check("err == nil", true, err == nil)
		checkInstance(t3, "t3(10, 0.50)", length, alpha, true)
	})
	...
	t.Run("volume factor = 0", func(t *testing.T) {
		...
		t3, err := NewT3ExponentialMovingAverageLength(&params)
		check("err == nil", true, err == nil)
		checkInstance(t3, "t3(10, 0.00)", length, alpha, true)
	})
	...
	t.Run("volume factor = 1", func(t *testing.T) {
		...
		t3, err := NewT3ExponentialMovingAverageLength(&params)
		check("err == nil", true, err == nil)
		checkInstance(t3, "t3(10, 1.00)", length, alpha, true)
	})
