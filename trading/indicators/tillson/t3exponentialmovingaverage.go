...
func newT3ExponentialMovingAverage(length int, alpha float64, v float64, firstIsAverage bool,
	...
	desc := "T3 exponential moving average " + name
...
func (s *T3ExponentialMovingAverage) Metadata() indicator.Metadata {
	return indicator.Metadata{
		Type: indicator.T3ExponentialMovingAverage,
		Outputs: []output.Metadata{
			{
				Kind:        int(T3ExponentialMovingAverageValue),
				Type:        output.Scalar,
				Name:        s.name,
...
func (s *T3ExponentialMovingAverage) UpdateScalar(sample *data.Scalar) indicator.Output {
	output := make([]any, 1)
...
func (s *T3ExponentialMovingAverage) UpdateBar(sample *data.Bar) indicator.Output {
	return s.UpdateScalar(&data.Scalar{Time: sample.Time, Value: s.barFunc(sample)})
}

// UpdateQuote updates the indicator given the next quote sample.
func (s *T3ExponentialMovingAverage) UpdateQuote(sample *data.Quote) indicator.Output {
	return s.UpdateScalar(&data.Scalar{Time: sample.Time, Value: s.quoteFunc(sample)})
}

// UpdateTrade updates the indicator given the next trade sample.
func (s *T3ExponentialMovingAverage) UpdateTrade(sample *data.Trade) indicator.Output {
